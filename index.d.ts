...
export interface IPresaleWallet {
  encseed: string;
  ethaddr: string;
  btcaddr: string;
  email: string;
...
export interface IScryptKdfParams extends IKdfParams {
...
export interface IPbkdf2KdfParams extends IKdfParams {
  prf: 'hmac-sha256';
  c: number;
}
...
export interface IEtherWalletLocked {
  private: string;
  encrypted: true;
  address: string;
  locked: true;
...
export interface IEtherWalletUnlocked {
  private: string;
  locked: false;
  encrypted: false;
  address: string;
...
export interface IPublicKeyOnlyWallet {
  _pubKey: string;
...
export interface IFullWallet extends IPublicKeyOnlyWallet {
  _privKey: string;
 ...
 */
export function generate(icap?: boolean): IFullWallet;
 ...
 */
export function generateVanityAddress(pattern: string | RegExp): IFullWallet;
 ...
 *
 */
export function fromPrivateKey(input: Buffer): IFullWallet;

/**
 * @description create an instance based on a BIP32 extended private key (xprv)
 * @param input
 */
export function fromExtendedPrivateKey(input: Buffer): IFullWallet;
 ...
 * @param input
 */
export function fromExtendedPublicKey(input: string): IPublicKeyOnlyWallet;
